...
## 책임 토폴로지: Brainstem through Prefrontal Cortex

| 계층 | 책임 | 변경 계약 |
| --- | --- | --- |
| **Brainstem** | 진실/비조작, 안전/보안, 출처/무손실, 결정론적 전이 | **절대 불변.** 상위 계층이나 역할이 덮어쓸 수 없습니다. |
| **Cerebellum** | 반복 가능한 자동화와 일정 | 통제된 변경 대상이며 예약 계층일 수 있습니다. |
| **Hippocampus** | 지속 상태와 메모리 | 명시적 소유권과 출처 아래 통제됩니다. |
| **Amygdala** | 위험 및 충돌 레지스트리 | 통제된 변경 대상이며 예약 계층일 수 있습니다. |
| **Cerebral Cortex** | 워크플로와 오케스트레이션 | 검토와 롤백이 가능한 통제 계층입니다. |
| **Default Mode Network** | 페르소나와 커뮤니케이션 행동 | 명시적인 변경 통제 메타데이터 아래 관리됩니다. |
| **Prefrontal Cortex** | 목표와 방향 | 명시적인 변경 통제 메타데이터 아래 관리됩니다. |

해부학 명칭은 **책임과 권한**을 나타내며 런타임 실행 순서나 컴파일 순서를 뜻하지 않습니다.

## 서로 독립적인 세 평면

1. **Brain 평면** — 책임, 권한, 변경 규칙
2. **Actor/Role 평면** — 역량, 입출력, 권한, 상태 범위, 에스컬레이션
3. **Compilation 평면** — 해부학적 책임 명칭과 독립된 명시적 의존성 DAG와 컴파일 순서
...
뉴런 런타임은 직교 실행·증거 평면이며 Brain이나 Role 권한을 만들지 않습니다.

| 요소 | 역할 |
| --- | --- |
| **Functional Neuron** | 타입 포트와 명시적 통합 임계치를 가진 역량 결합 처리 노드입니다. |
| **Synapse** | 흥분/억제 효과, 강도, 논리 지연을 가진 타입 연결입니다. |
| **Regulator** | 감쇠와 TTL이 있는 제한 조절값이며 Receptor 없이는 영향을 주지 않습니다. |
| **Receptor** | Regulator를 Neuron 임계치·이득에 제한 범위 안에서 연결합니다. |
| **Homeostat** | 측정값을 목표 범위로 되돌리는 제한된 음성 피드백입니다. |
| **Support** | 상태를 관찰하고 throttle·retry·quarantine을 제안만 합니다. |
| **Logical Clock** | 벽시계 권한이 없는 결정적 tick phase입니다. |
| **Plasticity Proposal** | 근거·롤백을 포함하지만 시뮬레이션이 적용하지 않는 변경안입니다. |
| **ActivationScenario** | 합성 입력 신호·측정값·tick/event 한계를 선언합니다. |
| **CompiledConnectome** | Brain 산출물에 결합된 결정적 회로 투영이며 권한 원천이 아닙니다. |
| **NeuralTrace** | 활성화·조절·Support 제안·종료 원인을 담는 불변 실행 증거입니다. |

[신경내분비 예제 설명](examples/neuroendocrine-public/README.md)과 [런타임 참조](docs/reference/neural-runtime.md)를 참고하십시오.
