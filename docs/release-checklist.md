# 0.2.0 release-readiness checklist
...
into an isolated environment and exercises `--version`, `validate`, `compile`, `render hermes`,
...
project's offline guarantee applies to validation, compilation, rendering, and simulation at runtime, not to
package installation.

Compatibility evidence MUST compare the exact legacy `CompiledBrainRole` and Hermes artifact bytes/SHA against
the pinned 0.1.x oracle. Neural artifact success MUST NOT substitute for legacy compatibility proof.
